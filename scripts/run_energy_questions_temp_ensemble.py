#!/usr/bin/env python3
"""
Temperature-ensemble proxy baseline for energy questions.

For each query:
- retrieve context once
- generate multiple answers at different temperatures
...
def parse_temps(value: str) -> List[float]:
    temps = []
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Temperature-ensemble proxy baseline")
    parser.add_argument(
        "--config",
    ...
    parser.add_argument(
        "--temps",
        default="0.2,0.7,1.0",
        help="Comma-separated temperatures (e.g., 0.2,0.7,1.0)"
    )
    parser.add_argument(
        "--agree_threshold",
        type=float,
    ...
    for item in questions:
        ...
        answers = [
            rag.llm.generate_with_context(query, context_texts, temperature=t)
            for t in temps
        ]
